...
try:
    ...
    #print("Imported from local source!")
except:
    print("Import failed")
    pass
...
class ChannelAttentionBHWC(nn.Module):
    def __init__(self, dim, reduction=4):
        super().__init__()
        # 针对轻量级网络，reduction 不宜过大
        mid_dim = max(dim // reduction, 4) 
    ...
    def forward(self, x):
        ...
        # 通道权重计算
        y = self.fc(y).view(b, 1, 1, c) # [B, 1, 1, C]
        return x * y
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.channel_attn = ChannelAttentionBHWC(self.d_inner, reduction=4)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # 1. 进入线性投影
        xz = self.in_proj(x)
        x, z = xz.chunk(2, dim=-1) # x: 左侧支路, z: 右侧支路 (BHWC)
        ...
        # --- 核心修改 2: 在右侧 z 支路应用通道注意力 ---
        # z 先经过注意力增强，再与左侧 y 结合
        z = self.channel_attn(z) # [B, H, W, D]
...
if __name__ == '__main__':
    # 1. 检查 GPU 是否可用，定义 device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"当前运行设备: {device}")

    # 2. 创建输入数据并转移到 GPU
    # 注意：如果 VSSBlock 内部有半精度运算，可以使用 .cuda().half()
    ...
    # 3. 实例化模型并转移到 GPU
    model = VSSLayer(16, 2).to(device)
    
    # 4. 如果是推理模式，建议加上 eval() 和 no_grad() 以节省显存
    model.eval()
    with torch.no_grad():
        y = model(x)
